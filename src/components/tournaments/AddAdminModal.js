...
import { sendHttpRequest, searchPlayer } from "../../common/Common";
...
class AddAdminModal extends React.Component {
  ...
  getAllPlayers() {
    sendHttpRequest("GET", "/player/").then((data) => {
      this.setState({
  ...
  updateAdminList() {
    ...
    sendHttpRequest("PUT", "/tournament/admins", null, JSON.stringify(data)).then((res) => {
